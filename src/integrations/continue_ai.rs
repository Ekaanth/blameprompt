/// Continue AI coding assistant integration for blameprompt.
///
/// Imports Continue session transcripts and converts them to blameprompt receipts.
///
/// Continue stores session data in:
///   ~/.continue/sessions/ (JSONL files)
///
...
/// A parsed Continue session.
#[derive(Debug)]
pub struct ContinueSession {
    pub session_id: String,
    pub model: String,
    pub messages: Vec<ContinueMessage>,
...
/// Locate the Continue sessions directory.
pub fn find_sessions_dir() -> Option<PathBuf> {
    let home = dirs::home_dir()?;

    let primary = home.join(".continue").join("sessions");
    if primary.exists() {
        return Some(primary);
    }
...
/// Parse a Continue session file.
pub fn parse_continue_session(path: &Path) -> Option<ContinueSession> {
        ...
        if let Some(tool_calls) = entry.get("toolCalls").and_then(|v| v.as_array()) {
            for tc in tool_calls {
                if let Some(name) = tc.get("name").and_then(|v| v.as_str()) {
                    ...
                    if let Some(args) = tc.get("args").or_else(|| tc.get("arguments")) {
                        if let Some(fp) = args
                            .get("file_path")
    ...
    // If JSONL parsing didn't work, try as single JSON
...
/// Import a specific Continue session file.
pub fn import_session(path: &Path) -> Option<Receipt> {
    let session = parse_continue_session(path)?;
...
/// Main entry point: scan Continue sessions and create receipts.
pub fn run_record_continue(session_path: Option<&str>) {
    let files = if let Some(path) = session_path {
        let p = PathBuf::from(path);
        if !p.exists() {
            eprintln!("[continue] File not found: {}", path);
            std::process::exit(1);
        }
        if p.is_dir() {
        ...
        match find_sessions_dir() {
            Some(dir) => {
                ...
                if files.is_empty() {
                    eprintln!("[continue] No session files found in {}", dir.display());
                    return;
                }
                files.into_iter().take(10).collect()
            }
            None => {
                eprintln!("[continue] Cannot find Continue sessions directory.");
    ...
    if count == 0 {
        eprintln!("[continue] No valid sessions found in the provided file(s).");
    } else {
        println!("[continue] Recorded {} Continue session(s)", count);
