/// Atlassian Rovo Dev AI coding agent integration for blameprompt.
///
/// Imports Rovo Dev session transcripts and converts them to blameprompt receipts.
///
/// Rovo Dev stores session data in:
///   ~/.rovo/sessions/ (JSONL files)
///
/// Each line has `role`, `content`, `model`, `timestamp`, `tools` array.
/// Config at ~/.rovo/config.json
...
#[derive(Debug)]
pub struct RovoDevSession {
    pub session_id: String,
    pub model: String,
    pub messages: Vec<RovoDevMessage>,
...
pub fn find_sessions_dir() -> Option<PathBuf> {
    ...
    let primary = home.join(".rovo").join("sessions");
    if primary.exists() {
        return Some(primary);
    }

    let root = home.join(".rovo");
    if root.exists() {
        return Some(root);
    }
...
pub fn parse_rovo_dev_session(path: &Path) -> Option<RovoDevSession> {
    ...
    for line in content.lines() {
        ...
        if let Some(tools) = entry.get("tools").and_then(|v| v.as_array()) {
            for tool in tools {
                if let Some(name) = tool.get("name").and_then(|v| v.as_str()) {
                    if !tools_used.contains(&name.to_string()) {
                        tools_used.push(name.to_string());
                    }
                    if let Some(args) = tool.get("args").or_else(|| tool.get("arguments")) {
                        if let Some(fp) = args
                            .get("file_path")
...
pub fn import_session(path: &Path) -> Option<Receipt> {
    let session = parse_rovo_dev_session(path)?;
...
pub fn run_record_rovo_dev(session_path: Option<&str>) {
    let files = if let Some(path) = session_path {
        let p = PathBuf::from(path);
        if !p.exists() {
            eprintln!("[rovo-dev] File not found: {}", path);
            std::process::exit(1);
        }
        if p.is_dir() {
        ...
        match find_sessions_dir() {
            Some(dir) => {
                ...
                if files.is_empty() {
                    eprintln!("[rovo-dev] No session files found in {}", dir.display());
                    return;
                }
                files.into_iter().take(10).collect()
            }
            None => {
                eprintln!("[rovo-dev] Cannot find Rovo Dev sessions directory.");
    ...
    if count == 0 {
        eprintln!("[rovo-dev] No valid sessions found in the provided file(s).");
    } else {
        println!("[rovo-dev] Recorded {} Rovo Dev session(s)", count);
