/// Sourcegraph Amp AI coding agent integration for blameprompt.
///
/// Imports Amp session transcripts and converts them to blameprompt receipts.
///
/// Amp stores session data in:
///   ~/.amp/sessions/ (JSONL files)
///
/// Each line has `type` (human/assistant/tool), `content`, `model`,
/// `tokensIn`, `tokensOut`, `timestamp`.
/// Config at ~/.amp/config.toml
...
pub fn find_sessions_dir() -> Option<PathBuf> {
    ...
    let primary = home.join(".amp").join("sessions");
    if primary.exists() {
        return Some(primary);
    }

    let root = home.join(".amp");
    if root.exists() {
        return Some(root);
    }
...
pub fn parse_amp_session(path: &Path) -> Option<AmpSession> {
    ...
    for line in content.lines() {
        ...
        // Token counts: Amp uses tokensIn/tokensOut
        if let Some(ti) = entry.get("tokensIn").and_then(|v| v.as_u64()) {
            input_tokens += ti;
        }
        if let Some(to) = entry.get("tokensOut").and_then(|v| v.as_u64()) {
            output_tokens += to;
        }

        // Also check standard usage object
...
pub fn import_session(path: &Path) -> Option<Receipt> {
    let session = parse_amp_session(path)?;
...
pub fn run_record_amp(session_path: Option<&str>) {
    let files = if let Some(path) = session_path {
        let p = PathBuf::from(path);
        if !p.exists() {
            eprintln!("[amp] File not found: {}", path);
            std::process::exit(1);
        }
        if p.is_dir() {
        ...
        match find_sessions_dir() {
            Some(dir) => {
                ...
                if files.is_empty() {
                    eprintln!("[amp] No session files found in {}", dir.display());
                    return;
                }
                files.into_iter().take(10).collect()
            }
            None => {
                eprintln!("[amp] Cannot find Amp sessions directory.");
    ...
    if count == 0 {
        eprintln!("[amp] No valid sessions found in the provided file(s).");
    } else {
        println!("[amp] Recorded {} Amp session(s)", count);
